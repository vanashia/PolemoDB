...
3: select * from (select distinct count(distinct relfilenode), relname from old_relfilenodes group by dbid, relname) as relfilenode_counts order by relname = 'reindex_crtab_part_heap_btree_id_idx' desc, relname;
3: COMMIT;
